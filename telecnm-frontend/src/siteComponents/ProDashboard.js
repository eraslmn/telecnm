...
const ProDashboard = () => {
    ...
    const uniqueApptInfo = Array.from(new Set(apptInfo.map(a => a.clientName)))
    .map(clientName => apptInfo.find(a => a.clientName === clientName));
    const joinCall = (appt)=>{
        console.log(appt);
        const token = searchParams.get('token');
                ...
                <div className="col-8">
                    ...
                    <div className="row num-2">
                        <div className="col-4 calendar">
                            <img src="https://cdn.pixabay.com/photo/2016/11/03/11/57/accountant-1794122_1280.png" />
                        </div>    
                    </div>
                </div>
